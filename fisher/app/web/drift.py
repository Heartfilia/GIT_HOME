...
from . import web
from ..froms.book import DriftForm
...
def send_drift(gid):
    ...
    if current_gift.is_yourself_fift(current_user.id):
    ...
    if request.method == "POST" and form.validate():
        ...
        send_mail(
            current_gift.user.email, '有人想要一本书', 'email/get_gift', wisher=current_user, gift=current_gift
        )
...
def save_drift(drift_form, current_gift):
    with db.auto_commit():
        ...
        drift_form.populate_obj(drift)   # 从验证表单中的信息复制到模型中,名字必须一样
